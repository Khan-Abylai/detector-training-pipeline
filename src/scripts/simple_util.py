...
filename = '/mnt/data/turkey/test.csv'
df = pd.read_csv(filename)
data_folder = '/mnt/data'
output_folder = '/home/user/detector_pipeline/data/turkey_data'
df.columns = ['filename']

df['full_path'] = df['filename'].apply(lambda x:os.path.join(data_folder, x))

for idx, item in df.iterrows():
    shutil.copy(item.values[1], os.path.join(output_folder, os.path.basename(item.values[0])))