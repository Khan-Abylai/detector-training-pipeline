...
init_annotation_path = '/mnt/workspace/data/filenames.txt'
df = pd.read_csv(init_annotation_path, header=None)
df.columns = ['filepath']
df["file_path"] = df['filepath'].apply(lambda x: x.replace('/mnt/workspace/data/facilities','')[1:])
df.drop(['filepath'], axis=1, inplace=True)

train, test = train_test_split(df, test_size=0.05, random_state=42)

train.to_csv('/mnt/workspace/data/train.txt', header=None, index_label=False, index=False)
test.to_csv('/mnt/workspace/data/val.txt', header=None, index_label=False, index=False)
